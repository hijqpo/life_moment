...
class AuthManagement {
  ...
  static Future<OperationResponse> signIn({email, password}) async{
    ...
    try {
        ...
        OperationResponse response = await initializeUserDocument(user);
  ...
  static Future<OperationResponse> signUp({@required email, @required password, String nickname}) async{
    ...
    try {
      ...
      OperationResponse response = await initializeUserDocument(user, nickname: nickname);
      return response;
    } 
    catch (error) {
  ...
  static Future<OperationResponse> initializeUserDocument(FirebaseUser user, {String nickname}) async{
    ...
    try{
      ...
      if (snapshot.documents.length > 0){
        ...
        await Firestore.instance.collection('users').add({
          ...
          'keywords': [nickname == null ? 'Anonymous' : nickname, user.email],
          'postCount': 0,
          'friendCount': 0,
          'score': 0,
          'gold': 0,
